        ...
        # FORGOT PASSWORD MODE
            ...
            if st.button("Recover Passkey"):
                if recover_user in st.session_state.team_keys:
                    # For demonstration purposes, we are showing the passkey directly. 
...
class OrpheusCommanderEngine:
    def __init__(self):
        ...
        self.model = "gemini-1.5-flash" # Updated to the standard real Gemini model to prevent API crashes
...
task_selection = st.sidebar.radio(
    ...
    (
        ...
        "🧠 Strategic Brainstorming",       # NEW CAPABILITY
        "💻 Code Generation & Debugging"   # NEW CAPABILITY
    )
)
...
if task_selection == "✉️ Handle Communications":
    ...
    if st.button("Generate Document"):
        with st.spinner("Commanding AI to draft your document..."):
            st.session_state.last_report = bot.prepare_report(topic, raw_data, format_type)
    ...
    if st.button("Initiate Brainstorm"):
        with st.spinner("Synthesizing strategic angles..."):
            st.markdown(bot.brainstorm(topic, goals))
    ...
    if st.button("Generate Code"):
        with st.spinner("Writing robust code..."):
            st.markdown(bot.write_code(language, requirements))
